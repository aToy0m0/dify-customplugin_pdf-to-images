...
services:
  plugin_daemon:
    environment:
      ...
      THIRD_PARTY_SIGNATURE_VERIFICATION_PUBLIC_KEYS: /app/storage/my_key_pair.public.pem
    volumes:
      # Mount your public key for signature verification
      - "./my_key_pair.public.pem:/app/storage/my_key_pair.public.pem:ro"
```

**Important Notes:**
- Place the public key file (`my_key_pair.public.pem`) in the same directory as `docker-compose.override.yaml`
...
cd /path/to/01_dify-pligins  # One level up from pdf-to-images/
...
./dify-plugin signature sign ./pdf-to-images.difypkg -p ./my_key_pair.private.pem

# 3. Verify signature (optional but recommended)
./dify-plugin signature verify ./pdf-to-images.signed.difypkg -p ./my_key_pair.public.pem
...
├── my_key_pair.private.pem        # Private key (generated by dify signature generate)
├── my_key_pair.public.pem         # Public key (generated by dify signature generate)
...
PRIVATE_KEY="./my_key_pair.private.pem"  # Use actual generated key names
PUBLIC_KEY="./my_key_pair.public.pem"
BUILD_DIR="./build"
...
| `THIRD_PARTY_SIGNATURE_VERIFICATION_PUBLIC_KEYS` | Path to public key for verification | `/app/storage/my_key_pair.public.pem` | Yes (prod) |
...
cd ../01_dify-pligins/  # Go up one level from pdf-to-images/
./dify-plugin plugin package ./pdf-to-images
```
...
./dify-plugin signature sign pdf-to-images.difypkg -p new_key_pair.private.pem
...
# And update volume mount: "./new_key_pair.public.pem:/app/storage/new_key_pair.public.pem:ro"
```
